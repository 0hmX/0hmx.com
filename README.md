...
- `src/page.html`: homepage, dark banner styling, and neutral styling with a red portrait.
- `src/assets/red-chrome-portrait.png`: supplied original artwork used for the profile avatar and browser tab icon; embedded by the build for single-file deployment.
...
This is an illustrative, stylized simulation rather than an engineering prediction. Vehicle artwork is AI-generated. The red chrome portrait is owner-supplied artwork, displayed in its original red. The surrounding theme uses neutral accents, with the same red portrait used as the favicon. The homepage introduces the autorouter and agent video framework, followed by work history at tscircuit, CashCowLabs, and as an independent engineer.
